#例4.2.1
process=read.csv("process.csv",fileEncoding = "UTF-8-BOM")#载入正交表及数据
X=process[,2:4]#记住第一列为行号，所以正交表列数要往后移一列，只取因子列/交互作用列
y=process[,6]#数据结果列
...
#例4.3.1
medicine=read.csv("medicine.csv",fileEncoding = "UTF-8-BOM")#载入正交表及数据
X=medicine[,c(2:5,8)]#记住第一列为行号，所以正交表列数要往后移一列，只取因子/交互作用列
y=medicine[,9]
#直观分析
...
#习题
antibiotics=read.csv("antibiotics.csv",fileEncoding = "UTF-8-BOM")#载入正交表及数据
X=antibiotics[,c(2:5,7)]
y=antibiotics[,9]
